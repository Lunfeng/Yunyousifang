...
public class lb_BirdController : MonoBehaviour {
	...
	bool BirdOffCamera(Vector3 birdPos){
		Vector3 centerPos = center.transform.position - birdPos;
		if (centerPos.x < -20 || centerPos.x > 20 || centerPos.z < -45 || centerPos.z > 45 || centerPos.y < -10 || centerPos.y > 20){
			return true;
		}else{
			return false;
		}
	...
	void SpawnBird(){
		if (!pause){
			...
			bird.transform.position = FindPositionAroundCenter();
	...
	Vector3 FindPositionOffCamera(){
		...
		Debug.DrawRay(currentCamera.transform.position, currentCamera.transform.position + (ray * dist));
		return currentCamera.transform.position+(ray*dist);
	}

	Vector3 FindPositionAroundCenter()
	{
		RaycastHit hit;
		float dist = Random.Range(20, 40);
		Vector3 ray = -center.transform.forward;
		int loopCheck = 0;
		ray += new Vector3(Random.Range(-.5f, .5f), Random.Range(-.5f, .5f), Random.Range(-.5f, .5f));
		while (Physics.Raycast(center.transform.position, ray, out hit, dist))
		{
			dist = Random.Range(2, 10);
			loopCheck++;
			if (loopCheck > 35)
		...
		return center.transform.position + (ray * dist);
	}
